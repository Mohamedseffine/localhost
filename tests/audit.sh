...
TMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/audit.XXXXXX")
SERVER_PID=
...
printf '%s\n' "$headers" | tr -d '\r' | grep -Eiq '^Content-Type: text/html; charset=utf-8$' || fail "static Content-Type"
printf '%s\n' "$headers" | tr -d '\r' | grep -Eiq '^Content-Length: [0-9]+$' || fail "static Content-Length"
pass "static response headers"

host_headers=$(curl -sS --resolve named.local:8080:127.0.0.1 -D - -o /dev/null http://named.local:8080/)
printf '%s\n' "$host_headers" | tr -d '\r' | grep -Fq 'X-Server-Name: named.local' || fail "virtual host selection"
...
cgi=$(curl -sS --http1.1 -H 'Transfer-Encoding: chunked' --data-binary 'chunked body' "$BASE_URL/cgi")
printf '%s\n' "$cgi" | grep -Fq 'DATA=chunked body' || fail "CGI chunked body"
pass "CGI chunked POST"

curl -sS -c "$TMP_DIR/cookies" -D "$TMP_DIR/h1" -o /dev/null "$BASE_URL/"
grep -Eiq '^Set-Cookie: session_id=[^;]+;' "$TMP_DIR/h1" || fail "session cookie creation"
curl -sS -b "$TMP_DIR/cookies" -D "$TMP_DIR/h2" -o /dev/null "$BASE_URL/"
if grep -Eiq '^Set-Cookie: session_id=' "$TMP_DIR/h2"; then fail "session cookie reuse"; fi
...
seq 1 100 | xargs -n 1 -P 10 sh -c 'curl -fsS -o /dev/null "$0/"' "$BASE_URL" || fail "concurrent GET probe"
